...
SET PARAM_VALUE = (SELECT TO_CHAR(MAX(RUN_DATE),'YYYYMMDD') FROM INF_B_DQ_LOAD_BATCH WHERE PROJECT_NAME = 'SC' AND STEP_NUM = 0)
WHERE PARAM_NAME = 'SC_RUN_DATE'
/
UPDATE INF_B_DQ_PARAMS 
SET PARAM_VALUE = (SELECT MAX(DQ_BATCH_SEQ) FROM INF_B_DQ_LOAD_BATCH WHERE PROJECT_NAME = 'SC' AND STEP_NUM = 0)
WHERE PARAM_NAME = 'SC_BATCH_SEQ'
/
COMMIT
/
