****************************************************************************************
PROGRAM:    STEP3 GENERATE REPORT_BMC.SAS
RE:         BMC REQUESTED THE DISINCENTIVE REPORT BE AGGREGATED (IN ADDITION TO THE
            CURRENT ACO BREAKOUT) THIS REPORT WILL BE GENERATED IN ADDITION TO
            BMC'S SCORECARD REPORT BY ACO.
            OUTPUTS EXCEL SPREADSHEET  MCO/MBH MEASURES 
...
NOTE:       GENERATES EXCEL SPREADSHEET. 
            TO DO: UPDATED MACRO VARIABLES: STRTDT,ENDDT,FT,MCO AND QUERY MONTH VALUE
...
proc sql;
connect to oracle (path=dwp1rac dbprompt=NO defer=NO user=tibbettsn password='tibbs2020#');

*TO BE UPDATED;
%LET STRTDT = JAN 2020;
%LET ENDDT  = APR 2020;
%LET TF = JAN2020_APR2020;
%LET MCO = BMC;

proc sort data=nw1.mco_mbh_pctdos_&TF. out=mco_mbh_pctdos_&TF.;
by cde_enc_mco dos_yrmonth;
where cde_enc_mco="&MCO";
run;

proc transpose data=mco_mbh_pctdos_&TF. /*(where=(cde_enc_mco='BMC'))*/
               out=mco_mbh_pctdos_&TF._t (drop=TOT_INPAT TOT_PHARM TOT_OUTPT TOT_OUTPTQ2 TOT_DENT TOT_DENTQ2 TOT_MED
...
select query_month, month_service, measure, claimtype, plan, claim_count, actual
from
(
select rpt1.*,
case when measure ='PCT_ID_MEDICAID' or measure ='PCT_DOS_FROM' or measure ='PCT_DOS_THRU' or 
...
inner join (select cde_enc_mco, dos_yrmonth, TOT_INPAT, TOT_PHARM, TOT_OUTPT, TOT_OUTPTQ2, TOT_DENT, TOT_DENTQ2, 
                   ...
                   TOT_RECORDS
            from mco_mbh_pctdos_&TF.
            /*where cde_enc_mco='BMC'*/)rpt2
on rpt1.plan=rpt2.cde_enc_mco and rpt1.month_service=rpt2.dos_yrmonth);
run;

proc sort data=report_step2 out=mco_mbh_report_&tf.;
by query_month plan month_service measure;
run;

proc print data=mco_mbh_report_&tf.;
var query_month month_service measure claimtype plan claim_count actual;
run;

proc export data=mco_mbh_report_&tf.
            (KEEP =query_month month_service measure claimtype plan claim_count actual where=(measure <> 'PCT_PROC_CODE'))
outfile="W:\APPLICATION ANALYSIS\MCO and MBHP Evaluation Scorecard\&TF.\REPORTS\MCO_MBH_SCORECARD_&MCO.&TF..XLS"
		      replace;			
sheet="&SYSDATE";
run;
...
proc sql;
create table nw1.mco_report_&MCO.&tf. as
select query_month, month_service, measure, claimtype, plan, claim_count, actual
