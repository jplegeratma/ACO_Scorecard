...
proc sql;
connect to oracle (path=dwp1rac dbprompt=NO defer=NO user=tibbettsn password=tibbs001);

*TO BE UPDATED;
%LET STRTDT = FEB 2017;
%LET ENDDT  = JAN 2018;
%LET TF = FEB2017_JAN2018;

proc sort data=nw1.mco_mbh_pcnt_&TF. out=mco_mbh_pcnt_&TF.;
by cde_enc_mco;
run;

proc transpose data=mco_mbh_pcnt_&TF.(where=(cde_enc_mco='BMC')) 
               out=mco_mbh_pcnt_&TF._t (where=(measure not in ('TOT_INPAT' 'TOT_PHARM' 'TOT_OUTPT' 'TOT_OUTPTQ2' 'TOT_DENT' 'TOT_DENTQ2' 'TOT_MED'
                      ...
                      'TOT_PHARMSCRIPT_NOTCOMP' 'TOT_RECORDS')))
name = Measure;
id cde_enc_mco;
run;
proc contents;
...
proc sql;
create table nw1.bmc_temp_report1 as
select
'2018-04' as query_month,
measure,
case when measure ='PCT_ID_MEDICAID' or measure ='PCT_DOS_FROM' or measure ='PCT_DOS_THRU' or 
          ...
          measure='PCT_QTY_UNIT_BILL' or measure='PCT_SVC_CAT' or measure='PCT_CLAIMCAT' or 
          measure='PCT_RECIND' or measure='PCT_AMTBILL' or measure='PCT_AMTALLOW' or measure='PCT_AMTPAY' or
		  measure='PCT_SERV_NPI' or measure='PCT_BILL_NPI'
	 ...
	 when measure='PCT_PRIMARY_DIAG' or measure='PCT_ICD_VERSION' or measure='PCT_POS_CODE' or measure='PCT_POS_TYPE' 
...
'BMC' as plan,
ROUND((BMC)*100,.01) as actual /*converting rate to a percent and rounding 2 decimal places*/
from mco_mbh_pcnt_&TF._t;
run;

proc print data = nw1.bmc_temp_report1;
run;

/*review the comments here,qa, then run*/
proc sql;
create table nw1.bmc_temp_report2 as
select rpt1.*,
case when measure ='PCT_ID_MEDICAID' or measure ='PCT_DOS_FROM' or measure ='PCT_DOS_THRU' or 
          measure ='PCT_SERV_PROV_ID' or measure ='PCT_SERV_PROV_ID_TYP' or measure ='PCT_SERV_PROV_TYP' or
          measure ='PCT_SERV_PROV_SPEC' or measure ='PCT_BILL_PROV_ID' or measure ='PCT_BILL_PROV_ID_TYP' or  
          measure='PCT_QTY_UNIT_BILL' or measure='PCT_SVC_CAT' or measure='PCT_CLAIMCAT' or 
          measure='PCT_RECIND' or measure='PCT_AMTBILL' or measure='PCT_AMTALLOW' or measure='PCT_AMTPAY'
		  then TOT_RECORDS
when measure='PCT_PRESCRIBE_PROV_ID' or measure='PCT_PRES_PROV_ID_TYP' or measure='PCT_SCRIPT_WRITTEN' or
          measure='PCT_REFILL' or measure='PCT_DISPENSE' or measure='PCT_SCRIPT' or measure='PCT_FEE' then TOT_PHARM_SCRIPT
when measure='PCT_NDC' then TOT_PHARMSCRIPT_NOTCOMP /*nt look into b/c don't see in sas composite*/
when measure='PCT_PRIMARY_DIAG' or measure='PCT_SERV_NPI' or measure='PCT_BILL_NPI' then TOT_NON_PHRM_DENTQ2 /**look into npi denom may be wrong**/
...
when measure='PCT_PROC_CODE_O' then TOT_OUTPTQ2+TOT_DENTQ2
...
from nw1.bmc_temp_report1 rpt1
inner join (select cde_enc_mco, TOT_INPAT, TOT_PHARM, TOT_OUTPT, TOT_OUTPTQ2, TOT_DENT, TOT_DENTQ2, 
                   ...
                   TOT_MOD_SURGERYM, TOT_INOUTLTC_NOART, TOT_INPT_OUTPT, TOT_PHARMSCRIPT_NOTCOMP,
                   TOT_RECORDS
            from mco_mbh_pcnt_feb2017_jan2018
            where cde_enc_mco='BMC')rpt2
on rpt1.plan=rpt2.cde_enc_mco;
run;

proc print data=nw1.bmc_temp_report2;
run;
...
proc sql;
create table NW1.SCOreport_&QTR. as
select 
...
case when measure ='PCT_ID_MEDICAID' or measure ='PCT_DOS_FROM' or measure ='PCT_DOS_THRU' or 
          ...
          measure='PCT_PAT_PAYAMT' or measure='PCT_CLAIMCAT' or measure='PCT_RECIND' or measure='PCT_AMTBILL' 
     then 'All Claim Types' 
     when measure='PCT_PRESCRIBE_PROV_ID' or measure='PCT_PRES_PROV_ID_TYP' or measure='PCT_SCRIPT_WRITTEN' or
          measure='PCT_REFILL' or measure='PCT_DISPENSE' or measure='PCT_SCRIPT' or measure='PCT_FEE' 
     then 'Pharmacy (prescriptions only, not OTC)'
...
case when measure in('PCT_ID_MEDICAID','PCT_DOS_FROM','PCT_DOS_THRU','PCT_SERV_PROV_ID','PCT_SERV_PROV_ID_TYP',
					  ...
					  'PCT_SCRIPT_WRITTEN','PCT_REFILL','PCT_DISPENSE','PCT_SCRIPT','PCT_FEE','PCT_NDC',
                      'PCT_CLAIMCAT','PCT_RECIND','PCT_AMTBILL'  
     then '98.0%'
...
proc export data=NW1.SCOreport_&QTR.
            (KEEP =Measure BHP CCA NAV SWH TFT UHC)
outfile="W:\APPLICATION ANALYSIS\PILOT SCO ENCOUNTERS REPORT CARD\&QTR.\JUL2017_SEP2017\MONTHLY DETAILS\SCO_REPORT_CARD_&QTR..XLS"
		      replace;			
sheet="&SYSDATE";
run;
